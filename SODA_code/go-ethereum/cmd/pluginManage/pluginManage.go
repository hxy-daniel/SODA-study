	...
	if monitor_arr, isTrue := plg.plugins[opcode]; isTrue {
		for index := 0; index < len(monitor_arr); index++ {
			...
			if plg.plugins[opcode][index].GetStatus(){
				...
				warning_level,results := ((plg.plugins[opcode])[index]).Send(data)	// 即monitor.Send()，即key为opcode的值，如EXTERNALINFOSTART:Handle_EXTERNALINFOSTART()
